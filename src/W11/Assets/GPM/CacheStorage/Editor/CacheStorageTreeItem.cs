        ...
        { 
            switch (column)
            {
                ...
                case CacheStorageTreeView.ColumnId.EXFIRES:
                    {
                        int freshnessLifetime = cacheInfo.CaculateFreshnessLifeTime();
                        if (freshnessLifetime > 0)
                        {
                            int sec = freshnessLifetime - cacheInfo.GetCurrentAge();
                            if (sec > 0)
                            {
                                EditorGUI.LabelField(cellRect, string.Format("{0}", Utility.GetTimeText(sec * TimeSpan.TicksPerSecond)));
                            }
                            else
                            {
